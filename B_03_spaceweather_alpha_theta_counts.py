...
"""
Count alpha/theta category occurrences for SPACE-WEATHER evaluation outputs.

Inputs (fixed):
  EVAL_FOLDER = "./data_spaceweather_evaluation_daily_global"
    └─ Global/<VAR>/
         pred_Global_<VAR>_<sy>_<ey>_w<...>_s<...>.csv
         columns: Date, close, alpha_pred, theta_pred

Two evaluation periods (space weather scope):
  - 1980-01-01 .. 2004-12-31
  - 2005-01-01 .. 2024-12-31

Per-year counts and log-scale yearly plots are produced for ALL variables found
under the evaluation folder (sunspot, f107, kp_daily_mean, Ap, ...).

Outputs (fixed) in ./data_spaceweather_statistics_global:
  # Alpha
  ...
  # Yearly plots (log y), one per VAR present:
...
def _plot_yearly_lines(df_overall_year_kind: pd.DataFrame,
                       ...
                       out_dir: Path):
    """Log-scale yearly line plot for one series_kind and one family (alpha/theta)."""
        ...
        if has_alpha:
            for tag, start, end in PERIODS:
                ...
                row = {
                    ...
                    "series_kind": kind,      # variable name
                    "file_start": sy,
                    "file_end": ey,
                    "period": tag,
